...
export default function Dashboard() {
  ...
  if (!loaded) return <div className="min-h-screen bg-[#070001] flex items-center justify-center"><div className="w-12 h-12 border-4 border-[#BE076D] border-t-transparent rounded-full animate-spin"></div></div>;
  ...
  const Input = ({l,v,on,tp='text'}:{l:string;v:string|number;on:(v:string)=>void;tp?:string}) => (
    <div><label className={`text-xs font-medium ${t2} mb-1.5 block`}>{l}</label><input type={tp} value={v||''} onChange={e=>on(e.target.value)} className={`w-full px-3 py-2.5 rounded-[var(--radius-input)] text-sm ${inp} border focus:border-[#BE076D] focus:ring-1 focus:ring-[#BE076D]/20 outline-none`}/></div>
  );
  const Select = ({l,v,on,opts}:{l:string;v:string;on:(v:string)=>void;opts:{v:string;l:string}[]}) => (
    <div><label className={`text-xs font-medium ${t2} mb-1.5 block`}>{l}</label><select value={v} onChange={e=>on(e.target.value)} className={`w-full px-3 py-2.5 rounded-[var(--radius-input)] text-sm ${inp} border focus:border-[#BE076D] outline-none`}>{opts.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}</select></div>
  );
  ...
  return (
    <div className={`min-h-screen ${bg}`}>
      ...
      <header className={`border-b ${bd} ${dark?'bg-[#070001]/80':'bg-white/80'} backdrop-blur-xl sticky top-0 z-40`}>
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src="/logo.svg" alt="RR" className="w-10 h-10 rounded-xl shadow-lg ring-1 ring-[#BE076D]/40"/>
            <div>
              <h1 className={`text-sm font-extrabold tracking-tight ${t}`}>RR<span className="text-[#BE076D]">ALIADOS</span></h1>
              <p className="text-[10px] text-[#D2C7D0] font-medium">Finanzas · corte {CORTE_ACTUAL}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Link href="/ops/clientes" className="px-3.5 py-2 rounded-xl text-xs font-semibold bg-[#DED116]/10 text-[#DED116] hover:bg-[#DED116]/20 transition-colors">Clientes</Link>
            <Link href="/ops/finanzas/crm" className="px-3.5 py-2 rounded-xl text-xs font-semibold bg-white/5 hover:bg-white/10 text-[#FFFFF3] transition-colors">CRM</Link>
            <Link href="/ops/finanzas/cuentas-cobro" className="px-3.5 py-2 rounded-xl text-xs font-semibold bg-[#BE076D]/15 text-[#E43A92] hover:bg-[#BE076D]/25 transition-colors">Cuentas</Link>
            <Link href="/ops/finanzas/documentos" className="px-3.5 py-2 rounded-xl text-xs font-semibold bg-white/5 hover:bg-white/10 text-[#FFFFF3] transition-colors">Documentos</Link>
            <Link href="/ops/finanzas/contabilidad" className="px-3.5 py-2 rounded-xl text-xs font-semibold bg-[#DED116]/10 text-[#DED116] hover:bg-[#DED116]/20 transition-colors">Contabilidad</Link>
            <label className="px-3.5 py-2 rounded-xl text-xs font-semibold bg-[#973D8F]/40 text-[#DED116] cursor-pointer hover:bg-[#973D8F]/60 border border-[#973D8F]/40 transition-colors">
              Sync Excel
      ...
      {/* TABS */}
      <div className={`border-b ${bd} sticky top-[53px] z-30 glass ${dark?'bg-[#070001]/80':'bg-white/80'}`}>
        <div className="max-w-7xl mx-auto px-4 flex gap-1 overflow-x-auto">
          {([['resumen','Resumen'],['gastos','Gastos'],['proyectos','Proyectos'],['servicios','Servicios'],['movimientos','Movimientos'],['pagos','Pagos'],['calendario','Calendario'],['brechas','Brechas']] as [string,string][]).map(([k,l])=>(
            <button key={k} onClick={()=>setTab(k as typeof tab)} className={`px-4 py-3 text-xs font-medium border-b-2 whitespace-nowrap tab-indicator ${tab===k?'active text-[#E43A92]':`border-transparent ${t2} hover:${t}`}`}>{l}</button>
          ))}
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        {/* RESUMEN */}
        {tab==='resumen' && <>
          <div className="relative overflow-hidden rounded-[var(--radius-hero)] mb-6 p-[1px] bg-gradient-to-br from-[#973D8F] via-[#BE076D]/60 to-[#BE076D]">
            <div className="rounded-[calc(var(--radius-hero)-1px)] p-5 md:p-6 bg-[#14050F]">
              <div className="absolute -top-24 -right-24 w-72 h-72 rounded-full bg-[#BE076D]/15 blur-3xl pointer-events-none"></div>
              <div className="absolute -bottom-24 -left-24 w-72 h-72 rounded-full bg-[#973D8F]/30 blur-3xl pointer-events-none"></div>
              <div className="relative flex flex-wrap items-start justify-between gap-4 mb-4">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-[#E43A92] font-bold mb-1">Pagos del 31 de agosto 2026</p>
                ...
                <div className="text-right">
                  <p className="text-3xl md:text-4xl font-extrabold tracking-tight text-[#E43A92] rr-num">{fmt(total31)}</p>
              ...
              <div className="relative grid grid-cols-1 md:grid-cols-3 gap-2 mb-4">
                <div className={`px-3 py-2.5 rounded-xl border ${bd} bg-white/[0.02]`}>
                  ...
                  <p className="text-base font-extrabold text-[#E43A92] rr-num">{fmt(candilejas31)}</p>
                ...
                <div className={`px-3 py-2.5 rounded-xl border ${bd} bg-white/[0.02]`}>
                  ...
                  <p className="text-base font-extrabold text-[#E43A92] rr-num">{fmt(nomina31)}</p>
                ...
                <div className={`px-3 py-2.5 rounded-xl border ${bd} bg-white/[0.02]`}>
                  ...
                  <p className="text-base font-extrabold text-[#DED116] rr-num">{fmt(total31 + supuestoMin)} – {fmt(total31 + supuestoMax)}</p>
              ...
              <div className="relative grid grid-cols-1 md:grid-cols-2 gap-2 max-h-64 overflow-y-auto">
                {pagos31.map(p => (
                  <button key={p.id} type="button" onClick={() => openModal('pago', p)} className={`text-left px-3 py-2 rounded-xl border ${bd} hover:bg-white/[0.04] flex justify-between gap-2`}>
                    ...
                    <span className="text-xs font-bold text-[#E43A92] rr-num flex-shrink-0">{fmt(p.monto)}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className="rr-card border border-[var(--border)] rounded-[var(--radius-card)] p-5 card-hover animate-fadeInUp stagger-2">
              ...
              <p className={`rr-num text-3xl font-extrabold tracking-tight ${runway>=8?'text-[#22c55e]':runway>=4?'text-[#f5c518]':'text-[#ef4444]'}`}>{runway}<span className="text-sm font-bold text-[#D2C7D0]"> meses</span></p>
              <p className={`text-xs ${t3}`}>Burn {fmt(burn)}/mes</p>
            </div>
            <div className="rr-card border border-[#BE076D]/20 rounded-[var(--radius-card)] p-5 card-hover animate-fadeInUp stagger-3">
              <p className="text-[10px] uppercase tracking-widest text-[#E43A92] mb-2 font-bold">A pagar 31/08</p>
              <p className="rr-num text-3xl font-extrabold tracking-tight text-[#E43A92]">{fmt(total31)}</p>
        ...
        {tab==='gastos' && <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className={`${card} border ${bd} rounded-2xl p-5`}>
              ...
              <p className="rr-num text-3xl font-extrabold text-[#E43A92]">{fmt(totalFijosMes)}</p>
              ...
              <div className="divide-y divide-white/[0.04]">
                {GASTOS_FIJOS_MENSUALES.map((g, i) => (
                  <div key={i} className="px-4 py-3 flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs font-bold text-[#E43A92] flex-shrink-0">{fmt(g.monto * (g.frecuencia === '2/mes' ? 2 : 1))}</span>
                  </div>
                ))}
                <div className="px-4 py-3 bg-white/[0.02] flex items-center justify-between">
                  ...
                  <p className="text-sm font-extrabold text-[#E43A92]">{fmt(totalFijosMes)}</p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-8">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              <div>
                ...
                <p className={`text-xs ${t3} mt-1`}>Total confirmado: <span className="font-bold text-[#E43A92]">{fmt(totalPersonas)}</span> · {personasPagos.length} colaboradores. Selecciona una persona para ver sus conceptos y generar su cuenta de cobro.</p>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div className="max-h-[480px] overflow-y-auto divide-y divide-white/[0.04]">
                  {personasPagos.map(p => (
                    <button key={p.persona} type="button" onClick={() => { setPersonaSel(p.persona); setCcStatus(''); }}
                      className={`w-full px-4 py-3 flex items-center justify-between gap-3 text-left transition-colors ${personaSel === p.persona ? 'bg-[#BE076D]/10 border-l-2 border-[#BE076D]' : 'hover:bg-white/[0.02]'}`}>
                      <div className="min-w-0">
                      ...
                      <span className="text-xs font-bold text-[#E43A92] flex-shrink-0">{fmt(p.total)}</span>
                    </button>
                  ))}
                  <div className="px-4 py-3 bg-white/[0.02] flex items-center justify-between">
                    <p className={`text-xs font-semibold ${t}`}>Total</p>
                    <p className="text-sm font-extrabold text-[#E43A92]">{fmt(totalPersonas)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className={`${card} border ${bd} rounded-2xl overflow-hidden lg:col-span-2`}>
                <div className={`p-4 border-b ${bd} flex items-center justify-between`}>
                  ...
                  {personaActual && <span className="text-sm font-extrabold text-[#E43A92]">{fmt(personaActual.total)}</span>}
                </div>
                {personaActual ? (
                  <div className="p-4">
                    <div className="space-y-2 mb-4">
                      {personaActual.items.map((item, i) => (
                        <div key={i} className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02]">
                          ...
                          <span className="text-xs font-bold text-[#E43A92] flex-shrink-0">{fmt(item.monto)}</span>
                        </div>
                      ))}
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <Link href="/ops/finanzas/cuentas-cobro" className="px-4 py-2.5 rounded-xl text-xs font-semibold bg-white/5 hover:bg-white/10 text-[#FFFFF3] transition-colors">

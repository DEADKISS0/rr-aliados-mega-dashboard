...
export default function ReporteOptimizacionesWidget() {
  ...
  if (loading && reports.length === 0) {
    return (
      <WidgetCard title="Reporte de Optimizaciones" icon="📋">
  ...
  return (
    <WidgetCard
      title="Reporte de Optimizaciones"
      icon="📋"
    ...
    >
      ...
      {reports.length === 0 ? (
        ...
        <ReportHistorySelector
          reports={reports}
          maxItems={5}
          variantName="Reporte de Optimizaciones"
          renderSummary={(report) => {
            const s = report.summary as Record<string, number | undefined>;
            if (!s) return null;
            const items = [
              { label: "Análisis", value: s.analisis_cualitativo ? "Sí" : "No", color: s.analisis_cualitativo ? "var(--success)" : "var(--danger)" },
              { label: "Recomend.", value: s.recomendaciones, color: "var(--ember)" },
              { label: "Riesgos", value: s.riesgos, color: "var(--danger)" },
              { label: "Acciones", value: s.acciones, color: "var(--warning)" },
              { label: "Áreas Crit.", value: s.areas_criticas, color: "var(--danger)" },
              { label: "Tendencias", value: s.tendencias, color: "var(--amber)" },
            ];
            return (

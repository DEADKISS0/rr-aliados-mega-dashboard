...
export default function LandingPage() {
  return (
    <main className="landing-shell">
      ...
      <section id="clientes" className="landing-section landing-section--alt"><div className="section-intro"><p className="eyebrow">02 / ALIANZAS</p><h2>Marcas con<br /><span>algo que decir.</span></h2><p>Una vista pública del trabajo y las oportunidades que estamos construyendo.</p></div><div className="client-subsection"><h3 className="client-subsection__title">Con prototipo</h3><div className="client-grid">{getClientsWithPrototype().map((client) => <article key={client.slug} className="client-row"><span className="client-status">{statusLabel[client.status]}</span><h3>{client.name}</h3><p>{client.industry}</p><span className="client-links">{client.prototypeUrl || client.website ? <a href={client.prototypeUrl || client.website} target="_blank" rel="noreferrer" aria-label={`Ver proyecto ${client.name}`}>↗</a> : null}{client.instagram ? <a href={client.instagram} target="_blank" rel="noreferrer" aria-label={`Instagram ${client.name}`} className="client-ig">IG</a> : null}</span></article>)}</div></div><div className="client-subsection"><h3 className="client-subsection__title">En construcción</h3><div className="client-grid client-grid--pending">{getClientsWithoutPrototype().map((client) => <article key={client.slug} className="client-row"><span className="client-status">{statusLabel[client.status]}</span><h3>{client.name}</h3><p>{client.industry}</p><span className="client-pending">FICHA EN CONSTRUCCIÓN</span></article>)}</div></div></section>

  ...
  return <main className="min-h-screen px-5 py-8 md:px-10">
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-6 flex gap-2 overflow-x-auto border-b border-white/10">
        {tabs.map((item) => <button key={item.id} onClick={() => { setTab(item.id); reset(); }} className={`whitespace-nowrap px-4 py-3 text-sm font-semibold ${tab === item.id ? 'border-b-2 border-[var(--primary)] text-[#E43A92]' : 'text-[var(--muted-foreground)]'}`}>{item.label}</button>)}
      </div>

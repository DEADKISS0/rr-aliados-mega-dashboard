...
export default function CuentasCobroPage() {
  ...
  const bg = dark ? 'bg-[#070001]' : 'bg-gray-50';
  const card = dark ? 'bg-[#0f0f15]' : 'bg-white';
  const bd = dark ? 'border-white/[0.06]' : 'border-gray-200';
  const t = dark ? 'text-[#FFFFF3]' : 'text-gray-900';
  const t2 = dark ? 'text-[#D2C7D0]' : 'text-gray-500';
  const t3 = dark ? 'text-[#6b6859]' : 'text-gray-400';
  const inp = dark ? 'bg-white/5 border-white/10 text-[#FFFFF3]' : 'bg-gray-50 border-gray-200 text-gray-900';
          ...
          <div className="space-y-4">
            ...
            <div className={`${card} border ${bd} rounded-2xl overflow-hidden`}>
              <div className={`p-4 border-b ${bd} flex items-center justify-between`}>
                <div className="flex items-center gap-2">
                  <User className="h-4 w-4 text-[#BE076D]" />
                  <h2 className={`text-sm font-semibold ${t}`}>Colaborador</h2>
                </div>
                <button onClick={crearPersona} className="px-3 py-1.5 rounded-lg text-xs font-bold bg-[#BE076D]/15 text-[#E43A92] hover:bg-[#BE076D]/25 transition-colors inline-flex items-center gap-1"><Plus className="h-3 w-3" /> Nuevo</button>
              </div>
              <div className="p-4 space-y-3">
                <select value={personaId} onChange={(e) => seleccionarPersona(e.target.value)} className={`w-full px-3 py-2.5 rounded-xl text-sm ${inp} border focus:border-[#BE076D] outline-none`}>
                ...
                <div className="pt-1">
                  <button
                    ...
                    className="w-full px-3 py-2.5 rounded-xl text-xs font-bold bg-[#973D8F] text-[#DED116] hover:bg-[#973D8F]/80 border border-[#973D8F]/40 transition-colors inline-flex items-center justify-center gap-1.5"
                  >
            ...
            <div className={`${card} border ${bd} rounded-2xl overflow-hidden`}>
              ...
              <div className="p-3 space-y-1.5">
                {templates.map((tmpl) => (
                  <button key={tmpl.id} onClick={() => aplicarTemplate(tmpl.id)} className={`w-full text-left px-3 py-2.5 rounded-xl text-xs font-medium transition-all ${dark ? 'hover:bg-white/5 text-[#FFFFF3]' : 'hover:bg-gray-100 text-gray-800'} border ${bd}`}>
                    {tmpl.nombre}
          ...
          <div className="space-y-4">
            ...
            <div className={`${card} border ${bd} rounded-2xl overflow-hidden`}>
              ...
              <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={cuenta.estado} onChange={(e) => setCuenta({ ...cuenta, estado: e.target.value as CuentaCobroPayload['estado'] })} className={`w-full px-3 py-2.5 rounded-xl text-sm ${inp} border focus:border-[#BE076D] outline-none`}>
              ...
              <div className={`px-4 pb-4`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={agregarLinea} className="px-3 py-1.5 rounded-lg text-xs font-bold bg-[#BE076D]/15 text-[#E43A92] hover:bg-[#BE076D]/25 transition-colors inline-flex items-center gap-1"><Plus className="h-3 w-3" /> Agregar concepto</button>
                </div>
                ...
                <div className="space-y-2">
                  {lineas.map((linea) => (
                    <div key={linea.id} className={`grid grid-cols-[1fr_64px_110px_110px_32px] items-center gap-2 p-2 rounded-xl border ${bd} ${dark ? 'bg-white/[0.02]' : 'bg-gray-50'}`}>
                      <input value={linea.concepto} onChange={(e) => actualizarLinea(linea.id, { concepto: e.target.value })} placeholder="Concepto (ej: Sesión de cámara)" className={`w-full px-3 py-2 rounded-lg text-sm ${inp} border outline-none focus:border-[#BE076D]`} />
                      <input type="number" min={0} value={linea.cantidad || ''} onChange={(e) => actualizarLinea(linea.id, { cantidad: Number(e.target.value) || 0 })} placeholder="1" className={`w-full px-2 py-2 rounded-lg text-sm text-center ${inp} border outline-none focus:border-[#BE076D]`} />
                      <div className="relative">
                        <span className={`absolute left-2 top-1/2 -translate-y-1/2 text-xs ${t3}`}>$</span>
                        <input type="number" min={0} value={linea.precio || ''} onChange={(e) => actualizarLinea(linea.id, { precio: Number(e.target.value) || 0 })} placeholder="0" className={`w-full pl-6 pr-2 py-2 rounded-lg text-sm ${inp} border outline-none focus:border-[#BE076D] text-right`} />
                      </div>
                      <span className="text-xs font-bold text-[#E43A92] text-right">{COP.format(subtotalLinea(linea))}</span>
                ...
                <div className={`mt-3 flex items-center justify-between px-3 py-2.5 rounded-xl ${dark ? 'bg-[#BE076D]/10 border border-[#BE076D]/20' : 'bg-red-50 border border-red-200'}`}>
                  <span className={`text-xs font-semibold ${t}`}>Total a cobrar</span>
                  <span className="text-lg font-extrabold text-[#E43A92]">{COP.format(totalLineas)}</span>
                </div>
              </div>
              ...
              <div className="px-4 pb-4">
                ...
                <textarea value={cuenta.notas || ''} onChange={(e) => setCuenta({ ...cuenta, notas: e.target.value })} className={`w-full min-h-16 px-3 py-2.5 rounded-xl text-sm ${inp} border focus:border-[#BE076D] outline-none`} />
              </div>
                ...
                <div className={`rounded-xl border p-4 transition-all ${cuenta.declaracionNoSubcontratacion !== undefined && cuenta.declaracionNoSubcontratacion !== null ? `${bd} ${dark ? 'bg-white/[0.03]' : 'bg-gray-50'}` : `${bd}`}`}>
                  <div className="flex items-center justify-between gap-3 mb-2">
                    ...
                    <button
                      ...
                      className={`relative w-12 h-6.5 rounded-full transition-colors flex-shrink-0 ${cuenta.declaracionNoSubcontratacion ? 'bg-[#BE076D]' : 'bg-gray-300'}`}
                      style={{ height: 26 }}
                    >
                  ...
                  {cuenta.declaracionNoSubcontratacion && (
                    <>
                      <label className={`flex items-start gap-2.5 p-3 rounded-lg border cursor-pointer transition-all mt-1 ${'border-[#BE076D]/40 bg-[#BE076D]/5'}`}>
                        <input
                          type="checkbox"
                          ...
                          className="mt-0.5 h-4 w-4 rounded accent-[#BE076D]"
                        />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_240px] gap-4">
              ...
              <div className="space-y-3">
                <button onClick={() => registrar()} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold bg-[#973D8F] text-[#DED116] hover:bg-[#973D8F]/80 border border-[#973D8F]/40 transition-colors">
                  <Save className="h-4 w-4" /> Registrar
                </button>
                <button onClick={descargarPdf} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold bg-gradient-to-r from-[#BE076D] to-[#973D8F] text-white hover:opacity-90 transition-opacity">
...
function F({ label, v, on, type = 'text', dark }: { label: string; v: string; on: (v: string) => void; type?: string; dark?: boolean }) {
  ...
  return (
    <div>
      <label className={`text-xs font-medium ${dark ? 'text-[#D2C7D0]' : 'text-gray-500'} mb-1.5 block`}>{label}</label>
      <input type={type} value={v} onChange={(e) => on(e.target.value)} className={`w-full px-3 py-2.5 rounded-xl text-sm ${inp} border focus:border-[#BE076D] focus:ring-1 focus:ring-[#BE076D]/20 outline-none`} />
    </div>
  );
}

function Preview({ cuenta, lineas, dark }: { cuenta: CuentaCobroPayload; lineas: ConceptoLinea[]; dark?: boolean }) {
  const paper = dark ? 'bg-[#FFFFF3] text-[#141414]' : 'bg-white text-gray-900';
  return (
    <article className={`min-h-[560px] rounded-2xl border border-[#BE076D]/20 p-8 shadow-2xl ${paper}`}>
      {/* Header */}
      <div className="flex items-start justify-between border-b-4 border-[#973D8F] pb-5 mb-6">
        <div>
          <p className="font-mono text-xs uppercase text-[#BE076D] font-bold">{cuenta.numero} · v{cuenta.version || 1}</p>
        ...
        <div className="flex flex-col items-end">
          <div className="w-16 h-16 rounded-xl bg-[#973D8F] flex items-center justify-center text-[#DED116] font-black text-lg">RR</div>
      ...
      <div className="grid grid-cols-2 gap-5 text-sm mb-6">
        <div>
          <p className="text-[10px] font-black uppercase text-[#BE076D] mb-1">Cobrador</p>
        ...
        <div>
          <p className="text-[10px] font-black uppercase text-[#BE076D] mb-1">Datos bancarios</p>
          ...
          <p className="text-[10px] font-black uppercase text-[#BE076D] mt-2 mb-1">Detalle</p>
      ...
      <div className="my-6 border-y-2 border-[#973D8F]/20 py-5">
        <p className="text-[10px] font-black uppercase text-[#BE076D] mb-2">Concepto</p>
          ...
          <div className="space-y-1.5">
            {lineas.map((l) => (
              <div key={l.id} className="flex items-center justify-between gap-3 text-xs">
                ...
                <span className="font-semibold text-[#973D8F] flex-shrink-0 w-24 text-right">{COP.format(subtotalLinea(l))}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="flex items-end justify-between mt-8">
        <div>
          <p className="text-[10px] font-black uppercase text-[#BE076D]">Valor a cobrar</p>
          <p className="text-4xl font-black text-[#973D8F]">{COP.format(cuenta.monto || 0)}</p>
        </div>
      </div>
      ...
      <div className="mt-8 grid grid-cols-2 gap-8">
        ...
        <div>
          <p className="text-[10px] font-black uppercase text-[#973D8F] mb-1">El colaborador</p>
        ...
        <div className="text-center">
          <p className="text-[10px] font-black uppercase text-[#973D8F] mb-1">Representante legal</p>
      ...
      {cuenta.declaracionNoSubcontratacion && (
        <div className="mt-6 rounded-lg border-2 p-4 border-[#973D8F]/30 bg-[#973D8F]/5">
          <p className="text-[10px] font-black uppercase mb-1.5 text-[#973D8F]">Declaración de no subcontratación · ✓ firmada</p>
